...
- Правила интерпретации окна:
  - окно учитывается **только** когда для слота заданы оба ключа (`start` и `end`);
  - неполная конфигурация (только один из ключей) игнорируется и слот не активирует;
  - `start`/`end` должны быть в диапазоне `0..23`, иначе окно игнорируется;
  - `start == end` считается невалидной конфигурацией и не активирует слот;
