...
2. Если задан explicit `npc_activity_route`, используется он.
3. Для `alert` сначала проверяется `npc_route_profile_alert`.
4. Иначе используется `npc_route_profile_slot_<current_slot>`.
5. Затем fallback в `npc_route_profile_default`.
6. Затем area cache fallback (`npc_route_cache_slot_<slot>` / `npc_route_cache_default`).
7. Иначе `default_route`.
