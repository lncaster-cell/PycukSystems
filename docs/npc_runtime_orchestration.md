...
> Примечание по расположению кода: active production runtime-скрипты оркестрации находятся в `src/modules/npc/`. Каталог `tools/*` рассматривается как legacy/reference для миграции; `src/modules/npc_behavior/` используется как redirect/документационный слой.
