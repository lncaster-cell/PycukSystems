...
> Примечание по расположению кода: production runtime-скрипты оркестрации находятся в `tools/npc_behavior_system/`. Каталог `src/modules/npc_behavior/` используется как redirect/документационный слой.
