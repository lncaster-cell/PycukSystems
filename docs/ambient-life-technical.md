...
Цель: обеспечить локальный прогрев/охлаждение area без введения централизованного оркестратора. На первом этапе ownership/adjacency хранятся только на area locals (без отдельного controller object).

**Минимальные locals на area:**
1. `al_quarter_id` (`string`) — квартал, к которому принадлежит area.
2. `al_adj_count` (`int`) + `al_adj_0..N` (`string`) — прямые соседи area в читаемом dense-формате.
3. `al_area_heat` (`int`) — текущее состояние прогрева:
   - `0` = `COLD`
   - `1` = `WARM`
   - `2` = `HOT`

Этого достаточно для локального резолва соседства «по месту», когда событие пробуждения/активности обрабатывается в конкретной area и распространяется максимум на 1 hop по adjacency. Для обратной совместимости допускается `al_adjacent_areas` (CSV), который разбирается каноничным parser-ом с кешированием в area locals.

### 6.2.2 Правила резолва соседства

1. Если area-источник получает/удерживает `HOT`, её прямые соседи из `al_adj_count` + `al_adj_0..N` (или CSV fallback) могут подниматься в `WARM`.
...
`Interior`-area по умолчанию фиксируется в `COLD`.

Исключения задаются только явно через area locals:
1. `al_interior_mode_whitelist=1`;
2. `al_mode_pin` (контентный pin явного режима).

Это правило нужно для стабилизации фоновой нагрузки: interior зоны не удерживаются в `WARM/HOT` только по историческому состоянию без explicit whitelist/pin.

### 6.2.4 Fallback при неполном adjacency-конфиге

При битом/неполном adjacency-конфиге (`al_adj_count`/`al_adj_*` или CSV `al_adjacent_areas`):
...
1. На route-waypoint сна (где `al_activity` указывает sleep-loop) задайте `al_bed_tag=<BED_ID>` и создайте пару waypoint `<BED_ID>_approach` и `<BED_ID>_pose`.
