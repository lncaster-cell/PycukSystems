...
- `al_route_index` (`0..1023`) — опциональный индекс точки маршрута;
  - для явного признака присутствия индекса используется dual-key контракт: сначала `al_route_index_present`, затем legacy `al_route_index_set`;
  - при наличии хотя бы одного валидного `al_route_index` в группе одного route-tag и любого флага присутствия включается indexed-режим;
  - waypoint без валидного индекса или без флага присутствия в indexed-режиме отбрасываются;
