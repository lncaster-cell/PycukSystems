...
### 4.4 Контракт времени для heartbeat-интервалов NPC
- Интервалы `npc_tick_interval_idle_sec` и `npc_tick_interval_combat_sec` трактуются как **целые секунды**.
- Минимально поддерживаемый интервал — `1` секунда; значения меньше `1` нормализуются к дефолту режима.
- Субсекундная точность для heartbeat-планирования в текущем runtime-контракте **не поддерживается**.
...
- Детальная спецификация схемы NPC, индексов, PRAGMA и write-behind: `docs/npc_persistence.md`.
