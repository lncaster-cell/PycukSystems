...
const int JOB_GUARD2 = 1009; // JOB: Stationary Guard. Should do nothing but stand there
...
const string PREY_RESREF = "sPreyResRef";
const string CHASED_RESREF = "sChasedResRef";
const string FRIEND_VAR_NAME = "X2_L_FRIEND";
const string PREY_MADE_VAR_NAME = "X2_L_PREYMADE";
const float FRIEND_FOLLOW_DISTANCE = 1.5f;
const string GO_AWAY_SHOUT_STRING = "Go away";
const string FACTORY_VAR_NAME = "X2_L_FACTORY";
const string MY_LOCAL_LOCATION_VAR_NAME = "lMyLocation";
const string MY_SPAWN_LOCAL_LOCATION_VAR_NAME = "lMySpawnLocation";
...
// * Creates a PREY creature (called by Prey 'see baddy' and Factory scripts)
void CreatePrey();

// * Creates a rat (called in x2_am_makerat, which is obsolete but preserved for legacy sake)
void CreateRat();
...
void SetJob( int nJob, object oObject = OBJECT_SELF, int nTownTalker = FALSE, string sResRef = "" )
{
    ...
    {
        //SpawnScriptDebugger();
		//PrettyDebug( "sResRef != BLANK" );

        object oCritter;
		//PrettyDebug( "Creating a CHASED!" );
        oCritter = CreateObject( OBJECT_TYPE_CREATURE, sResRef, GetLocation(OBJECT_SELF) );
        ...
        {
        	int i;
			//PrettyDebug( "Creating a some other critter!" );
    ...
    if ( nJob == JOB_TAG_NOTIT || nJob == JOB_TAG_IT )
    {
        ...
        {
            // * do nothing
            // * the IT creature will cause me to run in
...
void DoJob( object oSelf = OBJECT_SELF )
{
    ...
    switch ( GetJob() )
    {
		case 1000: // * Barmaid Behavior
			GoToRandomPersonAndInit(oSelf,"NW_L_NODRINKSFORME",10); 
			break;
        
        case 1001: // * Blacksmith
			PlayRandomImmobileAnimation();  
			break;
        
        case 1002: // * Patrol guard who talks to people
			GoToRandomPersonAndInit(oSelf,"X2_L_GIVEMEABREAK",10); 
			break;
        ...
        case 1006: // * Faithful companion: bonds to a nearby creature and will follow them
        {
            object oFriend = GetLocalObject( OBJECT_SELF, FRIEND_VAR_NAME );
			...
			if ( GetIsObjectValid(oFriend) == FALSE )
            {
                ...
                {
                    SetLocalObject( OBJECT_SELF, FRIEND_VAR_NAME, oCreature );
                }
            }
            else // * friend is valid, follow it
            {
                if ( GetCurrentAction(OBJECT_SELF) != ACTION_FOLLOW )
                {
                    ClearAllActions();
                    ActionForceFollowObject( oFriend, FRIEND_FOLLOW_DISTANCE );
                }
            }
         	break;
        }
		...
		case 1007: 
		case 1008: // * random walk (used by PREY and STRAY)
        {
...
void CreatePrey()
{
	string sPreyResRef = GetLocalString( OBJECT_SELF, PREY_RESREF );
   	object oPrey = CreateObject( OBJECT_TYPE_CREATURE, sPreyResRef, GetLocation(OBJECT_SELF) );

    SetLocalInt( OBJECT_SELF, PREY_MADE_VAR_NAME, GetLocalInt(OBJECT_SELF, PREY_MADE_VAR_NAME) + 1 );
    // * the rat stores the filth that created it
    SetLocalObject( oPrey, FACTORY_VAR_NAME, OBJECT_SELF);
    AssignCommand( oPrey, ActionMoveAwayFromObject(OBJECT_SELF) );
}

void CreateRat()
{
	// This function is now obsolete with the introduction of the 'gb_am_makeprey' functionality,
	// but we're preserving it for the sake of compilation in the older 'x2_am_makerat' script.
}
