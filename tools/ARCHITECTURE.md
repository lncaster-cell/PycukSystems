...
## 2.1) Требования системы и текущая реализация

Таблица фиксирует, что требовалось от системы, и как это реализовано сейчас.

| Требование | Реализация |
| --- | --- |
| Нулевая активность при отсутствии игроков. | Контроллер области не планирует тики при `p == 0`; при деактивации NPC скрываются/замораживаются. |
| Минимальная активность при наличии игроков. | Один таймер на область через `DelayCommand`, без отдельных тиков/таймеров на NPC. |
| Управление поведением только событиями. | Используются `EventUserDefined` + `SignalEvent`, события слота и `EVT_RESYNC`. |
| Отсутствие heartbeat на NPC и фоновых циклов. | Жизнедеятельность запускается только в OnEnter/OnExit/OnUserDefined; heartbeat для NPC не используется. |
| Никаких runtime-поисков по тегам/строкам. | Маршруты и ссылки кешируются, управление идёт через locals/реестр. |
| Все управляемые NPC должны быть в плотном реестре. | Реестр `n0..n99` + `n` обслуживается swap-remove и валидируется при обслуживании. |
...
- Требуются заранее заданные/закэшированные waypoint’ы (без поиска):
  - `AL_WP_PACE`
  - `AL_WP_WWP`
- Допустима схема по слотам с заранее заданными тегами на NPC:
...
`tools/al_system/al_acts_inc.nss`, чтобы хранить отображение ID → анимации без runtime-поисков.
