...
3. Скрипты Ambient Life из папки `tools/al_system/`.
...
2. Импортируйте все файлы из папки `tools/al_system/`:
...
Если вы используете внешний компилятор NWScript, добавьте `tools/` в include path
(`-i tools`), чтобы `#include "al_system/..."` корректно резолвился.
...
Другие механизмы (роли) не используются.
...
Для контроля порядка маршрута на каждом waypoint с нужным тегом задайте:

* локальный int `al_route_index` (0‑based, без пропусков/дубликатов);
* локальный int `al_route_index_set` = 1 (явный флаг, что индекс задан).

Именно по `al_route_index` сортируется маршрут; дубликаты пропускаются, а пропуски
фиксируются в debug‑логе области. Waypoint без `al_route_index_set` считается
неподготовленным и пропускается с сообщением `missing al_route_index; skipped`.
...
* `al_activity` → ID активности (см. `tools/al_system/al_acts_inc.nss`)
...
* В реестре на область максимум **100 NPC** (`AL_MAX_NPCS` в `tools/al_system/al_constants_inc.nss`).
* Если при `OnSpawn`/ресинке в область приходит NPC сверх лимита, его регистрация **отклоняется**:
  * NPC не попадает в `al_npc_<idx>`;
  * NPC не участвует в area-tick управлении;
  * в серверный лог пишется явная запись `AL: NPC registry overflow ...`;
  * на области увеличивается счётчик `al_npc_reg_dropped_total` (сколько регистраций было отброшено).
...
### 8.1 Подготовка масштабирования при росте NPC

Выберите один из вариантов (или комбинируйте):

1. **Увеличить лимит (`AL_MAX_NPCS`)**
   * Подходит для быстрых пилотов.
   * Риск: линейный рост цены проходов по dense-array в `AL_SyncAreaNPCRegistry` / broadcast.
2. **Сегментация на бакеты/подреестры**
   * Держать несколько реестров на область (например, по зонам, ролям или hash-bucket).
   * Каждый тик обрабатывать только активный поднабор; это снижает пиковую длину одного цикла.
3. **Controlled degradation policy**
   * При переполнении переводить «лишних» NPC в управляемый fallback (например, hide/idle, без маршрутов).
   * Обязательно оставлять наблюдаемость: метрики dropped/degraded + причины.
