...
- `al_route_index` — опциональный индекс точки маршрута (`0..1023`) для явного порядка.
  ...
  - Для legacy-контента поддерживается `al_route_index_set=1`.
  - Если хотя бы у одной waypoint с тегом маршрута задан валидный `al_route_index` (включая `0`) и есть явный флаг присутствия, тег работает в indexed-режиме.
