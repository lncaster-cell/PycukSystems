...
### GitHub Actions: сборка AL prototype (.ncs)
Workflow: `.github/workflows/nwn2-prototype.yml`.

Что делает пайплайн на `windows-latest`:
- запускает `compilator/nwn2_compiler/bin/NWNScriptCompiler.exe`;
- компилирует entrypoint-скрипты `scripts/al_prototype/al_*.nss` (исключая `*_inc.nss`) в 4 режимах: `check`, `analyze`, `opt`, `build`;
- использует include paths: `scripts/al_prototype` и `compilator/nwn2_compiler/stock_scripts`;
- сохраняет логи режимов в `out/logs/*.log`;
- публикует аннотации ошибок/предупреждений в UI GitHub Actions;
- добавляет краткий список диагностик в Job Summary.

Где смотреть результаты:
- **Artifacts → `al-npc-prototype-ncs`**: ZIP с `out/build/*.ncs` (только при успешном `build`);
- **Artifacts → `nwn2-compiler-logs`**: логи `check/analyze/opt/build` (загружаются всегда, даже при падении);
- **Summary / Annotations**: короткая сводка и кликабельные ошибки (если лог содержит file/line/col).

Как запустить:
- вручную через **Actions → Build AL NPC Prototype → Run workflow**;
- автоматически на `push`/`pull_request`, если изменены `scripts/al_prototype`, компилятор или workflow-скрипт.
