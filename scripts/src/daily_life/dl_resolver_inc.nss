...
int DL_ResolveDialogueMode(object oNPC, int nDirective, int nOverrideKind)
{
    if (nOverrideKind == DL_OVR_FIRE)
    {
        if (DL_GetNpcFamily(oNPC) == DL_FAMILY_LAW && (nDirective == DL_DIR_DUTY || nDirective == DL_DIR_HOLD_POST)) return DL_DLG_INSPECTION;
        return DL_DLG_HIDE;
    }
    ...
    {
        if (DL_GetNpcFamily(oNPC) == DL_FAMILY_LAW) return DL_DLG_INSPECTION;
        if (DL_GetNpcSubtype(oNPC) == DL_SUBTYPE_INSPECTION || DL_GetNpcSubtype(oNPC) == DL_SUBTYPE_GATE_POST) return DL_DLG_INSPECTION;
    ...
    if (nDirective == DL_DIR_UNASSIGNED) return DL_DLG_UNAVAILABLE;
    if (nDirective == DL_DIR_ABSENT) return DL_DLG_UNAVAILABLE;
    return DL_DLG_OFF_DUTY;
}
...
int DL_ResolveServiceMode(object oNPC, int nDirective, int nOverrideKind)
{
    ...
    if (nDirective == DL_DIR_UNASSIGNED) return DL_SERVICE_NONE;
    if (nDirective == DL_DIR_ABSENT) return DL_SERVICE_NONE;
